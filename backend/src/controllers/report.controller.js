  ...
  try {
    ...
    if (req.query.accountName) {
      ...
      if (accounts.length === 0) {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="transactions-empty.pdf"');
        const doc = new PDFDocument({ margin: 40, size: 'A4' });
        doc.pipe(res);
        doc.fontSize(16).text('No transactions found for the requested filters.', { align: 'center' });
        doc.end();
        return;
      }
    ...
    doc.fontSize(20).text('Debit/Credit Transactions Report', { align: 'center' });
    doc.moveDown();
    ...
    transactions.forEach((txn) => {
      doc
        ...
        .text(`Account: ${txn.account?.name ?? 'N/A'} (${txn.account?.type ?? '-'})`)
        .text(`Type: ${txn.type}`)
        .text(`Amount: ${txn.amount.toFixed(2)}`)
        .text(`Description: ${txn.description ?? '-'}`)
        .text(`Comment: ${txn.comment ?? '-'}`)
        .text(`Created via voice: ${txn.createdViaVoice ? 'Yes' : 'No'}`)
        .text(`Balance after transaction: ${txn.balanceAfterTransaction?.toFixed(2) ?? 'N/A'}`)
        .moveDown();
      doc.moveDown(0.5);
    });
